...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Prepara la consulta SQL para actualizar el nombre del grupo
    $sql = "UPDATE grupos SET nombre_grupo = ? WHERE id = ?";
    ...
    if (!$conexion->connect_error) {
        ...
        if ($stmt) {
            ...
            if ($stmt->execute()) {
                ...
                echo '<div class="alert alert-danger" role="alert">Error al actualizar el nombre del grupo: ' . $stmt->error . '</div>';
            }
...
$sql = "SELECT id, nombre_grupo FROM grupos WHERE id = ?";
$stmt = $conexion->prepare($sql);
...
if ($stmt) {
    ...
    $stmt->bind_result($id, $nombre_grupo);
    $stmt->fetch();
...
<!-- A continuación, muestra un formulario para editar el nombre del grupo con los datos actuales -->
